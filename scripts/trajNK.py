...
from trajUtils import b1Calc

class Trajectory:

    def __init__(self, quad, ctrlType, trajSelect, wpData):
        ...
        if (self.ctrlType == "xyz_pos"):
            ...
            # NK: Changed this self.averVel to adjust the time required for the trajectory
            if (self.averVel != 0):
                # NK: Commented out distance/time per segment calculation and replaced with my own on the subsequent lines
                # distance_segment = self.averVel*np.ones(np.shape(self.wps[1:]))
                # distance_segment = self.wps[1:] - self.wps[:-1]
                # self.T_segment = np.clip(np.sqrt(distance_segment[:,0]**2 + distance_segment[:,1]**2 + distance_segment[:,2]**2)/self.averVel,1,100) # NK: original is to divide by self.v_wp

                # NK: My attempt at distance/time per segment calculations
                n = len(wpData)
                for i in range(n-1):
                    wp1 = wpData[i,:]
                    wp2 = wpData[i+1,:]
                    dist = norm(wp2-wp1)
                    desTime = np.clip(2*dist/self.averVel,1,100)
                    if i == 0:
                        self.T_segment = np.array(desTime)
                    else:
                        self.T_segment = np.vstack((self.T_segment,desTime))

                self.t_wps = np.zeros(len(self.T_segment) + 1)
                self.t_wps[1:] = np.cumsum(self.T_segment)
                self.y_wps = np.zeros(len(self.T_segment) + 1) # NK: added to initialize yaw sp at zero, so initialization will run
            else: # NK: added this so it could run properly with the other changes made
        ...
        # NK: Get initial heading rate
        self.current_headingRate = quad.wDot[2]
        ...
        self.desJerk = np.zeros(3)   # NK: Added initialization for desired jerk
        self.desSnap = np.zeros(3)   # NK: Added initialization for desired snap
        ...
        self.des_rdot = 0.           # NK: initialize desired rdot
        self.sDes = np.hstack((self.desPos, self.desVel, self.desAcc, self.desThr, self.desEul, self.desPQR, self.desYawRate)).astype(float)

        # NK: Initialize the current desired b1d and b1d_dot arrays for the respective b1d_dot and b1d_2dot calculations
        self.current_b1d = np.zeros(3)
        self.current_b1d_dot = np.zeros(3)

        # NK: return the total reuired time for the trajectory based on the input desired average velocity
        tTotal = np.sum(self.T_segment)
        self.tTotal = tTotal
    ...
    def desiredState(self, t, Ts, quad):
        ...
        def pos_waypoint_min():
            ...
            # Stay hover at the last waypoint position
                ...
                # NK: Added additional states for desJerk and desSnap
        ...
        def yaw_waypoint_interp():
            ...
            if (t == 0) or (t >= self.t_wps[-1]):
                ...
                # NK: Calculate yaw accel
                delta_psiDot = self.desYawRate - self.current_headingRate
                self.des_rdot = delta_psiDot / Ts

                # Prepare next iteration
                self.current_heading = self.desEul[2]

                # NK: Prepare next iteration for yaw rate too
                self.current_headingRate = self.desYawRate
        ...
        '''NK: Get the desired state outputs'''
        # Unpack the states that are already formatted correctly
        xd = self.desPos
        xd_dot = self.desVel
        xd_2dot = self.desAcc
        xd_3dot = self.desJerk
        xd_4dot = self.desSnap
        eulDes = self.desEul
        Wd = np.array([0,0,self.desYawRate])
        Wd_dot = np.array([0,0,self.des_rdot])

        # Set e1 and e1_dot
        e1 = np.array([1,0,0])

        # Calculate the other desired states
        Rd = Rotation.from_euler('xyz', eulDes, degrees=False).as_matrix()
        b1d = b1Calc(Rd,e1)
        b1d_dot = (self.current_b1d - b1d) / Ts
        b1d_2dot = (self.current_b1d_dot - b1d_dot) / Ts

        # Set current b1d and current b1d_dot for next iteration
        if (t == 0):
            self.current_b1d = np.zeros(3)
            self.current_b1d_dot = np.zeros(3)
        else:
            self.current_b1d = b1d
            self.current_b1d_dot = b1d_dot


        # Pack things up and return the desired trajectory tuple
        desTraj = (xd, xd_dot, xd_2dot, xd_3dot, xd_4dot, b1d, b1d_dot, b1d_2dot, Rd, Wd, Wd_dot)
        return desTraj
